...
> **Web 最終交接版・精簡介面 UI1（2026-09-15）**：下載 `web-final-20260915-ui1`。下方只保留追蹤、速度與保持進度，其餘收進進階診斷；沿用使用者已接受的膝蓋回位防抖、10 秒上限與 42 類 Transformer v12，不改辨識邏輯、無須重訓。請依 [`組員換版說明`](docs/web_final_handoff_20260915.md) 操作。這是目前使用情境的驗收，不代表已完成跨人量化驗證；人物影片未上傳。
...
| **v13** | 安全邊界策略（pre-roll 0.60、回溯錨點、手腕 fallback）；E2 版面凍結 | 歷史 Windows 封存版 |
...
| **web-final-20260915** | Web 膝蓋回位防抖、每句重新定位、10 秒上限與即時診斷；沿用 v12 模型 | Web 辨識／切段基準；使用者本機驗收 |
| **web-final-20260915-ui1** | 精簡成追蹤、速度、保持進度；工程數據摺疊，計時按狀態顯示 | **目前 Web 交接版**；不改上述辨識／切段基準 |
...
**當時為什麼封存 v13 而不是更新的 Windows 版本。** v13 存在一項已定位的計時缺陷（合成時鐘導致錄影播放加速約 1.71 倍），但該缺陷的作用域限於錄影保真度與設定值語意，未改變送入模型的特徵契約，且時鐘在 v13 內部是自洽的——所有門檻皆於同一時鐘下實測校準，分段行為穩定。v15 則在離線 A/B 重播中出現整段漏切（12 段對 v13 的 14 段，並有 10.6 秒與 11.4 秒的完全不觸發空窗），段落召回劣於 v13。因此當時保留 v13 作為 Windows 封存基準；目前 Web 換版應使用上列 `web-final-20260915`，不可混用兩套切段設定。
