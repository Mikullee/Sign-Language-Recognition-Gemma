...
目前版本標籤：`web-final-20260915-ui1`（精簡介面）。辨識與切段沿用使用者已在 Chrome／攝影機環境接受的 `web-final-20260915` 基準；本次依使用者要求只改顯示，不改模型、後端或參數。原驗收限目前使用情境，不代表所有簽者或全部 42 句都已完成量化驗收。

這次交接包含「追蹤、速度、保持進度」三項主要資訊；操作提示保留在畫面上方，計時只在比劃／確認結束期間顯示，其餘收進預設摺疊的「進階診斷」。不再調整模型、辨識特徵或切段參數。

## 下載與選對版本

- [Release／下載頁](https://github.com/Mikullee/Sign-Language-Recognition-Gemma/releases/tag/web-final-20260915-ui1)
- [Web 完整原始碼＋模型 ZIP](https://github.com/Mikullee/Sign-Language-Recognition-Gemma/releases/download/web-final-20260915-ui1/Knee42-Web-Final-20260915-UI1.zip)
- [ZIP 的 SHA-256](https://github.com/Mikullee/Sign-Language-Recognition-Gemma/releases/download/web-final-20260915-ui1/Knee42-Web-Final-20260915-UI1.zip.sha256)
...
2. 將 ZIP 解壓到新的 `Knee42-Web-Final-20260915-UI1` 資料夾。
...
**從舊 Web 第一次升級時，前端＋Python 後端＋設定必須一起更新，不能只換 `index.html`。** 主要配套是 `webservice/`、`recognition/`、`configs/auto_trigger_knee_web_live.json` 及完整模型 bundle。只有已完整部署 `web-final-20260915` 的環境，這次 UI1 才可只替換 `webservice/static/index.html` 並清除快取；後端／模型／設定與該基底完全相同。若組員已有自訂網站外殼，不要直接蓋掉；將本包當作完整參考版，在其測試分支整合 `/stream` 的前後端契約，再驗收後上線。
...
- `http://127.0.0.1:8643/?v=web-final-20260915-ui1`：相機辨識頁面；不要用 `/replay.html` 取代相機頁。
...
- Python：426 項測試、另 4 subtests 通過；精簡介面前端 Node：24 項通過（含進階診斷空白鍵不誤觸錄製）。
